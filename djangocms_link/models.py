...
from djangocms_siteselector.models import SiteInstance
...
class AbstractLink(CMSPlugin):
    ...
    def link(self):
        if self.phone:
            ...
            clean_url = self.page_link.get_absolute_url()
            current_site = SiteInstance.objects.get_current()
            link = '/{0}{1}'.format(current_site.slug, clean_url)
        else:
            link = ''
